...
//
uint16_t led_duty = 0;
uint16_t servo_duty = 150;
uint16_t hex_ir = 0;
uint8_t IR_flag = 0;
uint16_t IR_signal[4] = {752,2800,3280,720}; // up,down,right,left
//
IR_Name Sony;
Led_Name led1;
Servo_Name servo1;
...
;
  ...
  /* USER CODE BEGIN 2 */
 //Device_Activate();
  IR_Init(&Sony,&htim1,TIM_CHANNEL_1);
  Led_Init(&led1,&htim2,TIM_CHANNEL_1);
    ...
    /* USER CODE BEGIN 3 */
		 if(IR_flag == 1){
		  	if(hex_ir == IR_signal[0]){
		  		Increase_Servo_Angle();
		  		Limit_Angle();
		  	}
		  	else if(hex_ir == IR_signal[1]){
		  		Decrease_Servo_Angle();
		  		Limit_Angle();
		  	}
...
static void MX_TIM1_Init(void)
{
  ...
  htim1.Instance = TIM1;
  htim1.Init.Prescaler = IR_PRESCALER;
